...
void				Bureaucrat::executeForm(AForm const &form){
	...
	{
		form.execute(*this);
		std::cout << YELLOW << this->getName() << RESET << BLUE << " executes " << RESET << YELLOW << form.getName() << RESET << std::endl;
	}
	catch (std::exception &e)
	{
		std::cout << YELLOW << this->getName() << RESET << BLUE << " cannot execute " << RESET << YELLOW << form.getName() << RESET << BLUE << " because: " << RESET << YELLOW << e.what() << RESET << std::endl;
	}
	return ;
}
